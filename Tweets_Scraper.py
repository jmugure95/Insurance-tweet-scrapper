...
# Goal: Collect Raw tweets from #Insurance
...
# This hashtag will be #Insurance provided as input but with this method, any hashtag can be used.
...
def search_for_hashtags(user_key, user_secret,access_token, access_token_secret, hashtag_phrase):
    ...
    InsuranceTweets = '_'.join(re.findall(r"#(\w+)", hashtag_phrase))


    # open the spreadsheet so as to write in it
    with open('%s.csv' % (InsuranceTweets), 'w', encoding= 'utf8') as file:
        ...
        # write tweets related to #Insurance (or any provided hashtag) to spreadsheet
